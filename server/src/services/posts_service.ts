...
export function addNewPost(post:any): boolean {
	const maxPostID = totalPosts.length;
	const title =  post.title;
	const text =  post.text;
	const authorid =  post.authorid;

	const user = getAllUsers().filter((u) => u.id === authorid);
	if (user === undefined || user.length ===0) return false;

	try {
		const newpost: Post = {
			id: (totalPosts.length + 1).toString(),
			title: title,
			text:  text,
			author: user[0],
		};
